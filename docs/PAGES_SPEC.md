...
| Content (Storefront) | **Единый редактор витрины** — см. требования владельца ниже |
| Plans & Licensing | Цены/лимиты/фичи планов и тексты лицензий — без деплоя |
| Customers | Юзеры: план, LTV, скачивания; фильтры; CSV. **Live с 2026-07-04**: список из D1 + смена ролей (customer/composer/admin) |
...
### 4.1 Требования владельца к контент-редактору (записано 2026-07-04, ещё не реализовано)

Модуль **Content (Storefront)** — одно место, где владелец управляет всей витриной:

- **Trending tracks** на главной: видеть текущий список, менять состав и порядок.
- **Коллекции и плейлисты**: просмотр, создание, редактирование. При создании — **кнопка «загрузить обложку»** (картинка → R2, без ручного копирования файлов в репо).
- **Теги в одном месте**: use cases, moods, жанры — управление списками и привязкой к трекам/коллекциям/плейлистам из одного экрана, без прыжков по разделам.
- **Прослушивание прямо в админке**: у каждого трека кнопка play (превью), чтобы собирать подборки на слух.
- **Версии треков**: alt-версии (30s, 15s, loop и т.д.) — это приложение к главному треку. Добавил мейн-версию в коллекцию/плейлист → версии едут автоматически, отдельно их не добавляют.

Дополнение к **Requests & Support**: в списке whitelist-заявок показывать **кто отправил** — email/имя аккаунта, его тариф, дату заявки, ссылку на канал (данные уже есть: whitelist_channels.user_id → users + subscriptions).

Решение владельца: админка остаётся **отдельной страницей** `/admin` (не встраивать модули в сайдбар /account). Из /account к ней ведёт ссылка «Admin panel» (видна только role=admin).

### 4.2 AI-фичи для клиентов (бэклог идей, записано 2026-07-05)

Принцип владельца: **никакой AI-генерации музыки** (позиционирование «real composers, no AI music»). AI — только чтобы помогать клиентам находить и использовать живую музыку. Технический фундамент общий: эмбеддинги каталога на Cloudflare Workers AI + Vectorize (без новых сервисов, дёшево).

1. **AI-поиск по сцене** — запрос человеческим языком («tense space trailer, no drums, under a minute») → треки каталога. Первый кандидат на реализацию.
2. **Референс-подбор** — «звучит как <известный трек/композитор>» → похожее из каталога, лицензионно чистое.
3. **Similar Tracks по-настоящему** — кнопка уже есть в UI как заглушка; те же эмбеддинги.
4. **Персональная лента под проект** — клиент описывает канал/проект, получает живую подборку + email «новые треки под ваш проект» (retention для подписки).
5. **Помощник по версиям** — «реклама 20 сек в Shorts» → сразу нужные cut-down версии.
6. **Claim-помощник** — по ссылке на видео сам определяет трек и заполняет заявку на снятие клейма.
7. **AI-ассистент по лицензиям** — чат строго по текстам лицензий/FAQ (без свободных юр-советов).
8. Для админки: авто-теги/BPM/mood при загрузке трека (нужен внешний аудио-анализ — позже).
