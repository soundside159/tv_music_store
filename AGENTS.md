...
- `functions/api/`: Cloudflare Pages Functions (backend). Live endpoints: `GET /api/health`, `GET /api/tracks` (returns `artist` = composer pseudonym; `?drafts=1` for admins also includes drafts/pending + `moderation_status`), `GET/POST /api/composer/tracks` (composer panel: own tracks + submit-for-review upload), `POST /api/auth/request-code`, `POST /api/auth/verify`, `POST /api/auth/logout`, `GET /api/me`. Shared helpers in `functions/api/_utils.ts` (Env bindings: DB, RESEND_API_KEY, EMAIL_FROM, STRIPE_*, GOOGLE_*). Email falls back to console logs until RESEND_API_KEY is set. Never commit secrets
