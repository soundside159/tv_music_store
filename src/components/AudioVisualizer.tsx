    ...
    const loop = (now: number) => {
      ...
      for (let i = 0; i < barCount; i++) {
        ...
        if (v > 0.008) {
          ...
          // color: gold↔white mix; solid by default, with an Opacity knob
          // in the tuning panel (owner asked for the handle back).
          const a = Math.max(0.05, (s.opacity ?? 100) / 100);
          ctx.fillStyle =
            goldShare >= 0.999
        ...
        if (capOn && pk > 0.006 && pk > v + 0.01) {
          ...
          if (restFade > 0.01) {
            ...
            ctx.fillStyle = `rgba(${GOLD},${(restFade * Math.max(0.05, (s.opacity ?? 100) / 100)).toFixed(3)})`; // cap follows the Opacity knob, fades only while resting
